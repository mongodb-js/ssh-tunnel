...
class SshTunnel extends EventEmitter {
  ...
  constructor(config: Partial<SshTunnelConfig> = {}) {
    ...
    this.server = socks5Server.createServer(async (info: any, accept: (intercept: true) => Socket, deny: () => void) => {
    ...
    if (!this.rawConfig.socks5Username) {
      ...
      this.server.useAuth(socks5AuthUserPassword((user: string, pass: string, cb: (success: boolean) => void) => {
        const success =
    ...
    (['close', 'error', 'listening'] as const).forEach(
      (eventName) => {
